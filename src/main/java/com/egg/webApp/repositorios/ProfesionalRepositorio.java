...
public interface ProfesionalRepositorio extends JpaRepository<Profesional, Long> {
    ...
    // @Query("SELECT DISTINCT p FROM Profesional p LEFT JOIN FETCH
    // p.turnosDisponibles LEFT JOIN FETCH p.calificaciones WHERE LOWER(p.nombre)
    // LIKE LOWER(CONCAT('%', :termino, '%')) OR LOWER(REPLACE(p.especialidad, ' ',
    // '_')) LIKE LOWER(CONCAT('%', :termino, '%'))")
    // List<Profesional> buscarPorNombreOEspecialidad(@Param("termino") String
    // termino);
